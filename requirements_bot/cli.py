...
@app.command()
def interview(project: str = typer.Option(..., prompt=True, help="Project name/title"),
    ...
    """
    asyncio.run(_run(project, out, model))

async def _run(project: str, out: str, model: str):
    session = await run_interview(project=project, model_id=model)
