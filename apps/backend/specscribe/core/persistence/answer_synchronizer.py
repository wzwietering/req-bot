from specscribe.core.database_models import AnswerTable
from specscribe.core.models import Answer, Session
...
class AnswerSynchronizer:
    ...
    def sync_answers(self, session: Session, merged_session, db_session) -> None:
        ...
        for answer in session.answers:
            ...
            if answer.question_id in existing_answers:
                self._update_existing_answer(existing_answers[answer.question_id], answer)
            else:
                self._add_new_answer(answer, session.id, db_session)

        self._remove_orphaned_answers(existing_answers, current_answer_question_ids, db_session)
    ...
    def _remove_orphaned_answers(self, existing_answers: dict, current_question_ids: set, db_session) -> None:
