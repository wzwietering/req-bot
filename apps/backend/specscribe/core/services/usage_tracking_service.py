...
class UsageTrackingService:
    ...
    def count_questions_in_window(self, user_id: str, days: int = 30) -> int:
        ...
        cached = self._get_cached_count(user_id)
        if cached is not None:
            return cached

        count = self._query_question_count(user_id, days)
        self._cache_count(user_id, count)
        return count
    ...
    def _get_cached_count(self, user_id: str) -> int | None:
        """Get cached count if valid, else None."""
        if user_id not in self._quota_cache:
            return None

        count, expires = self._quota_cache[user_id]
    ...
    def _cache_count(self, user_id: str, count: int) -> None:
        """Cache count with TTL."""
        expires = datetime.now(UTC) + timedelta(minutes=self._cache_ttl_minutes)
        self._quota_cache[user_id] = (count, expires)

    def _invalidate_cache(self, user_id: str) -> None:
        """Invalidate cached count for user."""
